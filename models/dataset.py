...
def apply_filter(img, f): # img, f - tensors
    new_img = img
    new_img = nashville(new_img, f[0])
    return new_img
...
def apply_filter_batch(img, labels):
    return [apply_filter(image, label) for image, label in zip(img, labels)]
...
#             image = apply_filters(image, label) 
